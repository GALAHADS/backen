...
const serviceAccount = require("/howard.json");

const firebaseAdmin = admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  databaseURL: "https://howard-9d223-default-rtdb.firebaseio.com"
});
...
module.exports = function(app) {
 ...
 /*   app.post('/addUser',function(req,res) {
        let userData = {
            "id": "-NJ48BB0XLKwbMJKI-Td",
            "email": "master@carrotmarket.com",
            "nickName": "밍",
            "phoneNum": "01077778888",
            "temperature": 45,
            "town": ["은평구", "강남구", "삼성동", "대화동", "탄현동"]
        };
    
        let usersRef = firebaseAdmin.database().ref('users');
        usersRef.push(userData, function(err) {    
            if (err) {
                console.error(err);
    ...
    let articleData = {
        ...
        "title": "물건 팔아요3333333~~!",
        "town": "강남구",
        "updateTime": 1670308550,
        "uploadTime": 1670308550
    };

    let articlesRef = firebaseAdmin.database().ref('articles');
    articlesRef.push(articleData, function(err) {    
        if (err) {
            console.error(err);
